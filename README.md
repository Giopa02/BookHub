...
🌐 **Application en ligne : [http://51.45.27.68](http://51.45.27.68)**
...
BookHub est déployé sur une instance **AWS EC2 (t3.micro)** sous Ubuntu.
...
| Serveur web | Nginx |
| PHP | PHP 8.4-FPM |
| Base de données | MySQL 9.6 via Docker |
| Administration BDD | phpMyAdmin via Docker |
| Hébergement | AWS EC2 (région eu-west-3) |

### Accès

| Service | URL |
|---|---|
| Application | [http://51.45.27.68](http://51.45.27.68) |
| phpMyAdmin | [http://51.45.27.68:8080](http://51.45.27.68:8080) |

### Lancer les containers Docker (si nécessaire)
```bash
cd ~/BookHub
...
```bash
# 1. Cloner le projet
git clone https://github.com/Giopa02/BookHub.git
cd BookHub
...
# 5. Configurer le mail (Mailtrap pour le développement)
MAIL_MAILER=smtp
...
MAIL_ENCRYPTION=tls
MAIL_FROM_ADDRESS=noreply@bookhub.fr
MAIL_FROM_NAME=BookHub
...
| MySQL | 9.6 | Base de données relationnelle |
...
| AWS EC2 | t3.micro | Hébergement en production |
| Mailtrap | - | Envoi d'emails 2FA (sandbox) |
...
│   │       └── UserController.php         # Utilisateurs / auth / 2FA
│   ├── Models/
│   │   ├── Author.php
│   │   ├── Book.php
│   │   ├── Borrow.php
...
│   │   ├── Role.php
│   │   ├── Status.php
│   │   └── User.php                       # Génération et reset du code 2FA
│   └── Providers/
...
│       └── ExampleTest.php
│
├── docker-compose.yml                     # MySQL + phpMyAdmin pour la production AWS
...
- **Déploiement sur AWS EC2** : la compatibilité PHP 8.4 avec Laravel a nécessité une configuration spécifique de Nginx et PHP-FPM. La connexion MySQL via Docker depuis Laravel nécessitait d'utiliser l'adresse IP du container plutôt que `localhost`.

- **Envoi d'emails 2FA en production** : les plateformes cloud testées bloquaient les connexions SMTP sortantes. Migration vers AWS EC2 avec Mailtrap (sandbox SMTP) pour contourner le problème.
...
- **Déploiement en production** sur AWS EC2 avec Nginx, PHP-FPM et MySQL via Docker
